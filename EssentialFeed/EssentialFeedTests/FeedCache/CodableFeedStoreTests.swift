    ...
    func test_retrieve_noSideEffectsOnNonEmptyCache() {
        ...
        sut.insert(feed, timestamp: timestamp) { insertionError in
            ...
            sut.retrieve { firstResult in
                ...
                default:
                    XCTFail("Expected retrieving twice from non empty cache to deliver same result with feed with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
                }
                exp .fulfill()
                }
            }
